<template>
  <div class="h-screen antialiased">
    ...
    <transition name="fade" appear>
      <div class="fixed inset-x-0 z-40 w-full h-full bg-black bg-opacity-70" v-show="showModal" @click="showModal = false"></div>
    </transition>
    <transition name="pop" appear>
      <div class="absolute inset-x-0 z-50 max-w-md p-10 mx-5 bg-white rounded-lg shadow-lg top-20 modal sm:mx-auto" v-show="showModal">
        <div class="flex items-center w-12 h-12 mb-2 rounded-full bg-gradient-to-tr from-yellow-100 to-blue-600">
        ...
        <h3 class="text-2xl font-extrabold tracking-tighter text-blue-600">Segnala attività</h3>
        ...
        <p class="mt-4 text-sm">Cerca attività</p>
        <input id="modalMapSearchBox" type="text" placeholder="Cerca attività" class="w-full p-4 mt-1 border border-gray-300 rounded-lg focus:outline-none" />
        <template v-if="formattedSelectedActivity && formattedSelectedActivity.name">
          <p class="mt-4 text-sm">Stai segnalando:</p>
          <div class="flex flex-col w-full bg-blue-50 rounded-2xl p-4 mt-4">
            <h2 class="leading-none text-base font-bold text-gray-800">{{ formattedSelectedActivity.name }}</h2>
            <h3 class="leading-none text-sm font-semibold text-gray-400 mt-1">{{ formattedSelectedActivity.address }}</h3>
          </div>
          <p class="mt-4 text-sm">Commento</p>
          <textarea type="text" v-model="notes" placeholder="Allega un commento (opzionale)" class="w-full p-4 mt-1 border border-gray-300 rounded-lg focus:outline-none"></textarea>
          <p class="mt-4 text-sm">E-mail</p>
          <input id="email-conferma" v-model="email" type="text" placeholder="Email" class="w-full p-4 mt-1 border border-gray-300 rounded-lg focus:outline-none" />
          ...
          <div class="flex flex-row mt-4 mb-4 space-x-2">
            <button @click="postReview" class="py-2 font-semibold text-white bg-blue-600 rounded-md px-7">
              Inoltra
            </button>
    ...
    <div class="flex flex-col items-start justify-between px-5 py-10 bg-white border-b border-gray-200 shadow-xl sm:py-5 sm:flex-row sm:items-center">
      ...
      <div class="flex flex-row items-center space-x-4">
        <button @click="showModal = !showModal" class="flex flex-row items-center py-3 space-x-3 font-semibold text-white bg-blue-600 border-t border-blue-400 rounded-md px-7 focus:ring-4">
    ...
    <div class="flex flex-row h-screen">
      ...
      <div class="flex items-center justify-center w-full h-full bg-gray-50">
        <GmapMap :center="{ lat: 41.89422107929427, lng: 12.436617991051996 }" ref="mapRef" :zoom="15" :streetViewControl="false" map-type-id="roadmap" style="width: 100%; height: 100%">
